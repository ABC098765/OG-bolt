...
const razorpay = new Razorpay({
  key_id: 'rzp_test_S6fv9Hd2UwHIbM',
  key_secret: 'NDLH7Nkp8ROgXJ6G9ZxNlx9B'
});
...
exports.notifyAdminsOnNewOrder = onDocumentCreated({
  ...
  console.log("[notifyAdminsOnNewOrder] Final admin notification message:", adminMessage);

  try {
    await admin.messaging().send(adminMessage);
    console.log("Successfully sent admin order notification:", adminMessage);
  } catch (error) {
  ...
  try {
    ...
    const userDoc = await admin.firestore()
      .collection("users")
      .doc(refreshedOrder.user_id)
      .get();

    const userData = userDoc.data();
    const fcmToken = userData && userData.fcm_token ? userData.fcm_token : null;

    console.log("[notifyAdminsOnNewOrder] User FCM token:", fcmToken);
    ...
    if (fcmToken) {
      ...
      await admin.firestore().collection('notifications').add({
        ...
        body: `Your order #${event.params.orderId} has been confirmed and is being processed.`,
        type: 'order_update',
        is_read: false,
      ...
      console.log("Successfully sent user order notification");
    } else {
      console.log("No FCM token found for user:", refreshedOrder.user_id);
    }
  } catch (error) {
    console.error("Error sending user order notification:", error);
  }
});